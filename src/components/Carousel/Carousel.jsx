...
const Carousel = () => {
  ...
  const slides = [
    { id: 1, image: "/images/banner1.jpg", alt: "Slide 1" },
      ...
      <div className="carousel w-full h-[420px] relative overflow-hidden mt-20">
        <div className="carousel-body h-full w-full">
          {slides.map((slide, index) => (
            <div
              key={slide.id}
              className={`carousel-slide absolute inset-0 w-full h-full flex items-center justify-center transition-opacity duration-700 ${index === currentSlide ? "opacity-100" : "hidden"
                }`}
            >
              <img
                src={slide.image}
